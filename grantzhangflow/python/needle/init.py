...
def one_hot(n, i, device=None, dtype="float32", requires_grad=False):
    ...
    device = ndl.default_device() if device is None else device
    arr = device.one_hot(n, i.numpy().astype("int32"), dtype=dtype)
    # print("one_hot",type(arr),)
    return ndl.Tensor(
        arr.numpy(),
        device=device,
...
def xavier_normal(fan_in, fan_out, shape=None, gain=1.0, **kwargs):
    ...
    shape = shape or [fan_in, fan_out]
    return randn(*shape, mean=0.0, std=a, **kwargs)
    ### END YOUR SOLUTION
...
def kaiming_uniform(fan_in, fan_out, shape=None, nonlinearity="relu", **kwargs):
    ...
    ### BEGIN YOUR SOLUTION
    a_map = {
      "relu":math.sqrt(2) * math.sqrt(3/fan_in),
    }
    a = a_map[nonlinearity]
    shape = shape or [fan_in, fan_out]
    return rand(*shape, low=-a, high=a, **kwargs)
    ### END YOUR SOLUTION
...
def kaiming_normal(fan_in, fan_out, shape=None, nonlinearity="relu", **kwargs):
    ...
    ### BEGIN YOUR SOLUTION
    a_map = {
      "relu":math.sqrt(2) / math.sqrt(fan_in),
    }
    a = a_map[nonlinearity]
    shape = shape or [fan_in, fan_out]
    return randn(*shape, mean=0.0, std=a, **kwargs)
