...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    ### BEGIN YOUR SOLUTION
    for node_index,node in enumerate(reverse_topo_order):
      ...
      # print("grad", node_index,node.op,grads)
      assert len(grads) == len(node.inputs), node.op
      for ins,this_grad in zip(node.inputs,grads):
        node_to_output_grads_list[ins] = node_to_output_grads_list.get(ins,[]) + [this_grad]
