...
class NDArray:
    ...
    def _init(self, other):
        ...
        self._strides = tuple(other._strides)
    ...
    def permute(self, new_axes):
        ...
        out = self.as_strided(new_shape,tuple(new_strides))
    ...
    def broadcast_to(self, new_shape):
        ...
        old_strides = list(self.compact_strides(old_shape))
        new_strides = []
        for index,(i,j) in enumerate(zip(old_shape,new_shape)):
          # assert i==j or (i==1)
          if i == j:
            new_strides.append(old_strides[index])
          else:
            assert i == 1
    ...
    def __getitem__(self, idxs):
        ...
        old_shape = list(self.shape)
        old_strides = list(self.strides)

        new_shape,new_strides,offset = [], [], 0
        for index,(i,j) in enumerate(zip(old_strides,slice_list)):
          new_shape.append(len(range(j.start,min(j.stop,old_shape[index]),j.step)))
          new_strides.append(i*j.step)
          offset += j.start * i

        return NDArray.make(
          tuple(new_shape),tuple(new_strides),
    ...
    def flip(self, axes):
        ...
        ### BEGIN YOUR SOLUTION
        if axes is None:
          axes = list(i for i in range(len(self.shape)))
        ...
        for index,i in enumerate(old_shape):
          if index in axes:
              offset += (i-1) * prod(old_shape[index+1:])
    ...
    def pad(self, axes):
        ...
        for old_shape,pad in zip(shape, padding):
          left,right = pad
          new_length = old_shape + left + right
          new_shape.append(new_length)
          new_slices.append(
              slice(left, left + old_shape,1)
          )
      
        out = full(
            new_shape,0.0, dtype=self.dtype,device=self.device
        )
        out[tuple(new_slices)] = self
        return out
        ### END YOUR SOLUTION
...
def stack(args,axis):
    t_args = [i for i in args]
    old_shape = [list(i.shape) for i in t_args]
    new_shape = []
    base_slice = []
    for index,i in enumerate(zip(*old_shape)):
      this_shape = i[0]
      if index == axis:
          new_shape.append(len(old_shape))
          base_slice.append(None)
      base_slice.append(slice(0,this_shape,1))
      new_shape.append(this_shape)

    out = empty(new_shape,dtype=t_args[0].dtype,
      device=t_args[0].device)
    
    for index,i in enumerate(t_args):
      clone_slice = base_slice[:]
      clone_slice[axis] = slice(index,index+1,1,)
      out[tuple(clone_slice)] = i
    return out
...
def split(a, axis):
    ...
    new_shape = []
    base_slice = []
    for index,i in enumerate(a_shape):
      if index!=axis:
        new_shape.append(i)
        base_slice.append(slice(0,i,1))
      else:
        new_shape.append(1)
        base_slice.append(None)
    out = []
    for i in range(a_shape[axis]):
      res = empty(new_shape,dtype=a.dtype,device = a.device)
      t_slice = base_slice[:]
      t_slice[axis] = i
      res = a[tuple(t_slice)].compact().reshape(new_shape)
      out.append(res)
    return tuple(out)
