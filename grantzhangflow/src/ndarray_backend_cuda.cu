...
__global__ void CompactKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
  ...
  if(gid < size){
    ...
    for(uint32_t i=0;i<length;i++){
      ...
      uint32_t this_dim = (gid / dims) % shape.data[i];
      n_dim += (strides.data[i] * this_dim);
    }
    out[gid] = a[n_dim]; 
  }
  /// END YOUR SOLUTION
...
__global__ void MatmulKernel(const scalar_t* a, const scalar_t* b, scalar_t* out, const size_t size,
              const uint32_t M, const uint32_t N, const uint32_t P) {
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid < size) {
    uint32_t row = gid / P;
    uint32_t col = gid - row * P;
    scalar_t ssum = 0;
    for(uint32_t i=0; i < N; i++){
      ssum += a[row*N+i] * b[i*P + col];
    }
    out[gid] = ssum;
  }
}
...
__global__ void ReduceMaxKernel(const scalar_t* a, scalar_t* out, const size_t size, const size_t reduce_size) {
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid < size) {
    size_t base_offset = gid * reduce_size;
    scalar_t smax = a[base_offset];
    for(size_t i=1;i<reduce_size;i++){
      smax = (smax > a[base_offset+i]) ? smax:a[base_offset+i];
    }
    out[gid] = smax;
  }
}
...
__global__ void ReduceSumKernel(const scalar_t* a, scalar_t* out, const size_t size, const size_t reduce_size) {
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid < size) {
    size_t base_offset = gid * reduce_size;
    scalar_t ssum = 0;
    for(size_t i=0;i<reduce_size;i++){
      ssum += a[base_offset + i];
    }
    out[gid] = ssum;
  }
}
